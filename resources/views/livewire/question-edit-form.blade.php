<!-- Formulaire d'édition de question dans le modal -->
...
<form wire:submit.prevent="save">
    ...
    <div class="mb-3">
        ...
        <input type="number" id="points" class="form-control" wire:model.live="points" min="1"
            max="100">
    ...
    <div>
            ...
            @foreach ($answers as $index => $answer)
                <div class="mb-3">
                    ...
                    <div class="d-flex align-items-center mb-2">
                        ...
                        <input type="text" class="form-control me-2" placeholder="Texte de la réponse"
                            wire:model.live="answers.{{ $index }}.answer_text">

                        @if ($type === 'single_choice')
                            <!-- Radio buttons pour choix unique -->
                            <div class="form-check me-2">
                                <input type="radio" class="form-check-input"
                                    name="correct_answer_{{ $index }}{{ $type }}"
                                    id="isCorrect{{ $index }}{{ $type }}" value="{{ $index }}"
                                    wire:model.live="correctAnswerIndex">
                                <label class="form-check-label"
                                    for="isCorrect{{ $index }}{{ $type }}">Correct</label>
                            </div>
                        @elseif($type === 'find_intruder')
                            <!-- Radio buttons pour trouver l'intrus -->
                            <div class="form-check me-2">
                                <input type="radio" class="form-check-input"
                                    name="intruder_answer_{{ $index }}{{ $type }}"
                                    id="isIntruder{{ $index }}{{ $type }}" value="{{ $index }}"
                                    wire:model.live="intruderAnswerIndex">
                                <label class="form-check-label"
                                    for="isIntruder{{ $index }}{{ $type }}">Intrus</label>
                            </div>
                        @elseif($type === 'multiple_choice')
                            <!-- Checkboxes pour choix multiple -->
                            <div class="form-check me-2">
                                <input type="checkbox" class="form-check-input"
                                    id="isCorrect{{ $index }}{{ $type }}"
                    ...
                    <div class="ms-4">
                        <textarea class="form-control" placeholder="Explication (optionnelle)" rows="2"
                            wire:model.live="answers.{{ $index }}.explanation"></textarea>
                    </div>

                    @error("answers.$index.answer_text")
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                    @error("answers.$index.explanation")
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
            @endforeach
        ...
        @else
            <div class="alert alert-info">
                <i class="bi bi-info-circle"></i> Pour les questions de type texte, les étudiants
                répondront librement sans réponses prédéfinies.
            </div>
        @endif
    </div>
    ...
    <div class="mt-4">
        <button type="submit" class="btn btn-success" wire:loading.attr="disabled">
            ...
            <span wire:target="save" wire:loading.remove>
                <i class="bi bi-check-circle"></i> Mettre à jour la question
            </span>
        </button>
        <button type="button" class="btn btn-warning" wire:click="cancelEdit">
            <i class="bi bi-x-circle"></i> Annuler l'édition
        </button>
    </div>
</form>
<script>
